...
describe('Pipeline - Build AMD', function() {
	...
	it('should build js files to multiple AMD modules and their source maps', function(
		done,
	) {
		var stream = vfs
			.src('src/js/foo.js')
	...
	it('should build js files to multiple AMD modules without their source maps', function(
		done,
	) {
		var stream = vfs.src('src/js/foo.js').pipe(
			buildAmd({
				base: path.resolve(),
				sourceMaps: false,
			}),
		...
		stream.on('end', function() {
			...
			assert.deepEqual(
				['dep/dep.js', 'metal/src/js/foo.js'],
				files.sort(),
			);
			done();
		});
	...
	it("should build js files correctly when base folder doesn't contain node_modules", function(
		done,
	) {
		var stream = vfs
			.src('src/js/foo.js')
	...
	it('should use given moduleName for the original source files', function(
		done,
	) {
		var stream = vfs
			.src('src/js/foo.js')
	...
	it('should use import path as module id when it has the "module:" prefix', function(
		done,
	) {
		...
		stream.on('data', function(file) {
			if (file.relative === 'metal/src/js/moduleAlias.js') {
				...
				assert.notStrictEqual(
					-1,
					contents.indexOf("define(['myModuleId']"),
				);
